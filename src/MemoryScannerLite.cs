...
// Supports both 32-bit and 64-bit process scanning
using System;
...
namespace DetectorLite
{
    public static class MemoryScannerLite
    {
        ...
        private const int CHUNK_SIZE_64 = 256 * 1024;  // 256KB for 64-bit
        private const int CHUNK_SIZE_32 = 128 * 1024;  // 128KB for 32-bit
        ...
        private const long MAX_ADDRESS_64 = 0x7FFFFFFFFFFFFFFF;
        private const long MAX_ADDRESS_32 = 0x7FFFFFFF;
        ...
        private static bool IsProcess64Bit(Process process)
        {
            ...
            if (Environment.Is64BitProcess)
            {
                ...
                // Assume 64-bit if detection fails on 64-bit OS
                return true;
            }
            // Scanner is 32-bit, can't detect 64-bit processes
            return false;
        }
        ...
        public static List<string> ScanProcess(Process process)
        {
            ...
            bool isTarget64Bit = IsProcess64Bit(process);
            bool isScanner64Bit = Environment.Is64BitProcess;

            // Check compatibility
            if (!isScanner64Bit && isTarget64Bit)
            {
                Console.WriteLine($"  Cannot scan 64-bit process (PID {process.Id}) from 32-bit scanner");
                return hits;
            }

            Console.WriteLine($"  Scanning PID {process.Id} ({process.ProcessName}) - {(isTarget64Bit ? "64-bit" : "32-bit")}...");

            // Set architecture-specific limits
            long maxAddress = isTarget64Bit ? MAX_ADDRESS_64 : MAX_ADDRESS_32;
            int chunkSize = isTarget64Bit ? CHUNK_SIZE_64 : CHUNK_SIZE_32;
            ...
            try
            {
                ...
                while (VirtualQueryEx(hProcess, addr, out mbi, mbiSize) > 0)
                {
                    ...
                    // Architecture-specific address validation
                    if (nextAddr <= currentAddr || nextAddr > maxAddress)
                        break;
                    ...
                    regionsScanned++;
                    totalBytes += ScanRegion(hProcess, mbi.BaseAddress, (int)regionSize, chunkSize, hits, earlyStopHits);
                }
            }
            catch (Exception ex)
            {
        ...
        private static long ScanRegion(IntPtr hProcess, IntPtr baseAddr, int regionSize, int chunkSize, List<string> hits, HashSet<string> earlyStopHits)
        {
            long totalRead = 0;
